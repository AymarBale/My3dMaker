...
public class Rules {
    ...
    public static void ApplyTextRule(ArrayList<Tracker> trackers , String rule, Line line){
        String[] parts = rule.split(" "); // Split the rule string by space
        if (parts.length != 3) {
            System.out.println("Invalid input format: " + rule);
            return;
    ...
    public Rules(String rule,int initX,int initY,int endX,int endY){
        this.rule = rule;
        this.initX = initX;
        this.initY = initY;
        this.endX = endX;
    ...
    public String toString() {
        return String.format("Rule: %s, Initial Position: (%d, %d), Final Position: (%d, %d)",
                rule, initX, initY, endX, endY);
    }
